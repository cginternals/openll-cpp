...
namespace
{
    ...
    std::unique_ptr<openll::GlyphRenderer> g_renderer;
    std::unique_ptr<openll::GlyphVertexCloud> g_cloud;
    ...
    {
        std::string name;
        std::function<void(std::vector<openll::Label>&)> function;
    };
    ...
    {
        {"Constant",                                 openll::layout::constant},
        {"Random",                                   openll::layout::random},
        {"Greedy",                                   std::bind(openll::layout::greedy,                  _1, openll::layout::standard, glm::vec2(0.2f))},
        {"Discrete Gradient Descent",                std::bind(openll::layout::discreteGradientDescent, _1, openll::layout::standard, glm::vec2(0.2f))},
        {"Simulated Annealing",                      std::bind(openll::layout::simulatedAnnealing,      _1, openll::layout::standard, glm::vec2(0.f))},
        {"Simulated Annealing with padding",         std::bind(openll::layout::simulatedAnnealing,      _1, openll::layout::standard, glm::vec2(0.2f))},
    };
}
...
std::vector<openll::Label> prepareLabels(openll::FontFace * font, glm::ivec2 viewport)
{
    std::vector<openll::Label> labels;
    ...
    for (int i = 0; i < g_numLabels; ++i)
    {
        ...
        sequence.setAlignment(openll::Alignment::LeftAligned);
        sequence.setLineAnchor(openll::LineAnchor::Ascent);
        sequence.setFontSize(fontsize);
        sequence.setFontFace(font);
        sequence.setFontColor(glm::vec4(glm::vec3(fontcolor), 1.f));
        sequence.setSuperSampling(openll::SuperSampling::Quincunx);
        ...
        const auto placement = openll::LabelPlacement{ glm::vec2{ 0.f, 0.f }
            , openll::Alignment::LeftAligned, openll::LineAnchor::Baseline, true };
...
openll::Label prepareHeadline(openll::FontFace * font, glm::ivec2 viewport, const std::string & name)
{
...
std::vector<openll::Label> getSequences(const std::vector<openll::Label> & labels)
{
    ...
    for (const auto & label : labels)
    {
        ...
        {
            sequences.push_back(openll::applyPlacement(label));
        }
    }
    return sequences;
}

void preparePointDrawable(const std::vector<openll::Label> & labels, PointDrawable& pointDrawable)
{
...
void prepareRectangleDrawable(const std::vector<openll::Label> & labels, RectangleDrawable& rectangleDrawable)
{
    std::vector<glm::vec2> rectangles;
    for (const auto & label : labels)
    {
        auto sequence = openll::applyPlacement(label);
        auto extent = openll::Typesetter::rectangle(sequence, glm::vec3(label.pointLocation, 0.f));
...
void runAndBenchmark(std::vector<openll::Label> & labels, Algorithm algorithm)
{
    ...
    std::cout << "Evaluation results for " << algorithm.name << ":" << std::endl
        ...
        << "  Upper Right:           " << positions[openll::RelativeLabelPosition::UpperRight] << std::endl
        << "  Upper Left:            " << positions[openll::RelativeLabelPosition::UpperLeft]  << std::endl
        << "  Lower Left:            " << positions[openll::RelativeLabelPosition::LowerLeft]  << std::endl
        << "  Lower Right:           " << positions[openll::RelativeLabelPosition::LowerRight] << std::endl;
    std::cout << std::endl;
}
...
void initialize()
{
    ...
    openll::FontLoader loader;
    g_font = std::unique_ptr<openll::FontFace>(loader.load(dataPath + "/fonts/opensansr36/opensansr36.fnt"));
    ...
    g_renderer = std::unique_ptr<openll::GlyphRenderer>(new openll::GlyphRenderer);
    g_cloud = std::unique_ptr<openll::GlyphVertexCloud>(new openll::GlyphVertexCloud);
    ...
    auto raw = openll::RawFile(dataPath + "/geodata/world.8192.4096.rgba.ub.raw");
